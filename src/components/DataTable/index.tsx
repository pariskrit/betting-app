...
export const liveColumns = (leagueName: string): any[] => [
  ...
  },
  {
    ...
    header: () => <HeaderTable text={"1"} />,
    cell: ({ row }: { row: Row<CustomResultData> }) => {
      return <ColumnTable text={row.original.odd_data.home_od} data={BET_SLIP_DATA[0]}/>;
    },
  },
  {
    accessorKey: "X",
    header: () => <HeaderTable text={"X"} />,
    cell: ({ row }: { row: Row<CustomResultData> }) => {
      return <ColumnTable text={row.original.odd_data.draw_od} data={BET_SLIP_DATA[1]} />;
    },
  },
  {
    accessorKey: "2",
    header: () => <HeaderTable text={"2"} />,
    cell: ({ row }: { row: Row<CustomResultData> }) => {
      return <ColumnTable text={row.original.odd_data.away_od} data={BET_SLIP_DATA[2]}/>;
    },
  },
];
